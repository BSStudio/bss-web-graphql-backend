...
vi.mock('postgraphile/presets/amber', () => ({
  PostGraphileAmberPreset: vi.fn().mockName('PostGraphileAmberPreset'),
}))
...
describe('postgraphile', () => {
  it('should configure PostGraphile v5 with the v4 compatibility preset', async () => {
    expect.assertions(6)
    ...
    expect.soft(mockPostgraphile).toHaveBeenCalledWith({
      extends: [
        ...
        mockMakeV4Preset.mock.results[0]?.value,
